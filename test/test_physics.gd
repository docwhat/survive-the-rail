...
func test_full_train_braking_distance_is_longer():
	var light_stop: float = Physics.stopping_distance(
		10.0,
		Physics.effective_deceleration(100.0, 10.0),
	)
	var heavy_stop: float = Physics.stopping_distance(
		10.0,
		Physics.effective_deceleration(100.0, 50.0),
	)
